...
<html lang="en">
    <head>
        ...
        <script src="https://kit.fontawesome.com/bee51370cb.js" crossorigin="anonymous"></script>        <title>Modulo ventas - SB Admin</title>
                        ...
                        <div class="card mb-4">
                            <div class="card-body">
                                <div class="search-box">
                                    <fom action="#">
                                        <div class="search">
                                            <input type="text" class="searchPalabra" placeholder="Buscar">
                                        ...
                                        <c:forEach var="v" items="${listaVentas}">
                                            <tr>
                                                ...
                                                <c:forEach var="c" items="${listaClientes}">
                                                    <c:if test="${v.getId_cliente()==c.getId_cliente()}">
                                                        <td>${c.getNombre()}</td>
                                                    </c:if>
                                                </c:forEach>
                                                <c:forEach var="ve" items="${listaVendedores}">
                                                    <c:if test="${v.getId_vendedor()==ve.getId_vendedor()}">
                                                        <td>${ve.getNombre()}</td>
                                                    </c:if>
                                                </c:forEach>
                                                <td>${v.getNit()}</td>
                                                <td>
                                                    <form action="Controlador">
                                                        <input type="hidden" name="idven" value="${v.getId_venta()}"/>
                                                        <button type="submit" class="btn-danger" name="accion" value="verDetalleVenta"><i class="fa fa-exclamation"></i> Ver productos </button><br><br>
                                                    </form>
                                                </td>
                                                <td>${v.getTotal()}</td>
        ...
        <script>
            ...
            if (venta === "detalle") {
                swal({
                    ...
                    text: "<%=request.getAttribute("ventas")%>",
                    icon: "success",
                    button: "Continuar"
                });
            }
